...
int main( int argc, char **argv ) {
					...
					switch( j ) {
						...
						case 2:
							...
							if ( opt == 'g' )
								...
								options.coordinator.push_back( tmpOption );
							break;
					}
				}
				break;
	...
	if ( ! coordinator->init( path, options.global, options.coordinator, verbose ) ) {
...
usage:
	fprintf(
		...
		"  -o, --option       Override the options in the config file of coordinator (coordinator.ini)\n"
