	...
	switch( event.type ) {
		...
		case CLIENT_EVENT_TYPE_SWITCH_PHASE:
		{
			...
			if ( event.message.switchPhase.toRemap ) {
				...
				coordinator->remapMsgHandler->transitToDegraded( servers, event.message.switchPhase.forced );
			} else {
				coordinator->remapMsgHandler->transitToNormal( servers, event.message.switchPhase.forced );
			}
			delete servers;
		}
			break;
