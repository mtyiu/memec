...
public class MemEC {
	...
	public boolean connect( boolean enableNamedPipe ) {
		try {
		...
		int bytes = this.protocol.generateHeader(
			...
			enableNamedPipe ? Protocol.PROTO_OPCODE_REGISTER_NAMED_PIPE : Protocol.PROTO_OPCODE_REGISTER,
			0, 0, qid
		);
		try {
		...
		if ( bytes == Protocol.PROTO_HEADER_SIZE ) {
			...
			System.err.println( "MemEC.connect(): [Error] Header length mismatch: " + bytes + " vs. " + this.protocol.header.length + "." );
			return false;
		}

		if ( this.protocol.header.isSuccessful() && this.protocol.header.opcode == Protocol.PROTO_OPCODE_REGISTER_NAMED_PIPE ) {
			try {
				bytes = this.read( this.protocol.header.length );
			} catch( IOException e ) {
				System.err.println( "MemEC.connect(): [Error] Unable to read NAMED_PIPE response from client." );
				return false;
			}
	...
	public String get( byte[] key, int keySize ) {
		...
		if ( bytes == this.protocol.header.length ) {
			...
			System.err.println( "MemEC.get(): [Error] Header length mismatch: " + bytes + " vs. " + this.protocol.header.length + "." );
		}
		return null;
	}
	...
	public boolean set( byte[] key, int keySize, byte[] value, int valueSize ) {
		...
		if ( bytes == this.protocol.header.length ) {
			...
			System.err.println( "MemEC.set(): [Error] Header length mismatch: " + bytes + " vs. " + this.protocol.header.length + "." );
		}
		return false;
	}
	...
	public boolean update( byte[] key, int keySize, byte[] valueUpdate, int valueUpdateOffset, int valueUpdateSize ) {
		...
		if ( bytes == this.protocol.header.length ) {
			...
			System.err.println( "MemEC.update(): [Error] Header length mismatch: " + bytes + " vs. " + this.protocol.header.length + "." );
		}
		return false;
	}
	...
	public boolean delete( byte[] key, int keySize ) {
		...
		if ( bytes == this.protocol.header.length ) {
			...
			System.err.println( "MemEC.delete(): [Error] Header length mismatch: " + bytes + " vs. " + this.protocol.header.length + "." );
		}
		return false;
	}
}
