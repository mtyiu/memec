...
public class ProtocolDirect {
	...
	public static final byte PROTO_ADDRESS_SIZE           = 6;
	...
	public ProtocolDirect( int keySize, int chunkSize ) {
		this.buf = new byte[ ProtocolDirect.getSuggestedBufferSize( keySize, chunkSize ) ];
	...
	public int generateAddressHeader( byte magic, byte to, byte opcode, int id, int addr, short port ) {
		int ret = this.generateHeader( magic, to, opcode, PROTO_ADDRESS_SIZE, id );

		this.buf[ ret     ] = ( byte )( ( addr >> 24 ) & 0xFF );
		this.buf[ ret + 1 ] = ( byte )( ( addr >> 16 ) & 0xFF );
		this.buf[ ret + 2 ] = ( byte )( ( addr >>  8 ) & 0xFF );
		this.buf[ ret + 3 ] = ( byte )( ( addr       ) & 0xFF );
		this.buf[ ret + 4 ] = ( byte )( ( port >>  8 ) & 0xFF );
		this.buf[ ret + 5 ] = ( byte )( ( port       ) & 0xFF );

		ret += PROTO_ADDRESS_SIZE;
