...
public class OneMeasurementHistogram extends OneMeasurement
{
  ...
  public static final String BASE="histogram.baseUnit";
  public static final String BUCKETS_DEFAULT="1000";
  public static final String BASE_DEFAULT="1";
  ...
  public OneMeasurementHistogram(String name, Properties props)
  {
    ...
    baseUnit=Integer.parseInt(props.getProperty(BASE, BASE_DEFAULT));
  ...
  {
    if (latency/baseUnit>=_buckets)
    {
      histogramoverflow++;
    }
    else
    {
      histogram[latency/baseUnit]++;
    }
    operations++;
  ...
  public void exportMeasurements(MeasurementsExporter exporter) throws IOException
  {
    ...
    for (int i=0; i<_buckets; i++)
    {
      ...
      {
        exporter.write(getName(), "95thPercentileLatency("+Integer.toString(baseUnit)+"us)", i);
        done95th=true;
      }
      if (((double)opcounter)/((double)operations)>=0.99)
      {
        exporter.write(getName(), "99thPercentileLatency("+Integer.toString(baseUnit)+"us)", i);
		done99th=true;
        break;
      }
    }

	/* workaround for exporter test */
    if ( !done95th )
        exporter.write(getName(), "95thPercentileLatency("+Integer.toString(baseUnit)+"us)", -1);
    if ( !done99th )
        exporter.write(getName(), "99thPercentileLatency("+Integer.toString(baseUnit)+"us)", -1);
