...
public:
	...
	// Remapping SET
	char *reqRemappingSet( size_t &size, uint32_t id, uint32_t listId, uint32_t chunkId, char *key, uint8_t keySize, char *value, uint32_t valueSize, char *buf = 0, uint32_t sockfd = UINT_MAX, bool isParity = false, struct sockaddr_in *target = 0 );
	// GET
