...
for s in $sizes; do
	...
	for iter in {1..10}; do
		...
		for n in {7..14}; do
			mkdir -p ${BASE_PATH}/results-multi/value_sizes/memec/$s/$TIMESTAMP/hpc$n
			cp ${BASE_PATH}/results-multi/value_sizes/tmp/hpc$n/$s/*.txt ${BASE_PATH}/results-multi/value_sizes/memec/$s/$TIMESTAMP/hpc$n
