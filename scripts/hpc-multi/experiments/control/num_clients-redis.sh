	...
	for c in $clients; do
		for iter in {1..10}; do
			...
			for w in $workloads; do
				...
				for i in $(seq $c); do
					n=$(expr $i + 6)
					ssh hpc$n "screen -S ycsb -p 0 -X stuff \"${BASE_PATH}/scripts-multi/experiments/client/value_sizes-redis.sh $s $w $c $(printf '\r')\"" &
				done
			...
			for i in $(seq $c); do
				n=$(expr $i + 6)
				mkdir -p ${BASE_PATH}/results-multi/num_clients/redis/$c/$s/$TIMESTAMP/hpc$n
				cp ${BASE_PATH}/results-multi/value_sizes/tmp/hpc$n/$s/*.txt ${BASE_PATH}/results-multi/num_clients/redis/$c/$s/$TIMESTAMP/hpc$n
