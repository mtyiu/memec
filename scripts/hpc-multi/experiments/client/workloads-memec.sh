...
if [ $w == "load" ]; then
	echo "-------------------- Load (workloada) --------------------"
	${BASE_PATH}/scripts-multi/ycsb/memec/load.sh $t 2>&1 | tee ${BASE_PATH}/results-multi/workloads/tmp/${HOSTNAME}/$c/$t/load.txt
else
	echo "-------------------- Run ($w) --------------------"
	${BASE_PATH}/scripts-multi/ycsb/memec/run.sh $t $w 2>&1 | tee ${BASE_PATH}/results-multi/workloads/tmp/${HOSTNAME}/$c/$t/$w.txt
fi
