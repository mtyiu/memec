...
if [ $w == "load" ]; then
	echo "-------------------- Load (workloada) --------------------"
	${BASE_PATH}/scripts-multi/ycsb/memec/load-size.sh $s $c 2>&1 | tee ${BASE_PATH}/results-multi/value_sizes/tmp/${HOSTNAME}/$s/load.txt
else
	echo "-------------------- Run ($w) --------------------"
	${BASE_PATH}/scripts-multi/ycsb/memec/run-size.sh $s $w $c 2>&1 | tee ${BASE_PATH}/results-multi/value_sizes/tmp/${HOSTNAME}/$s/$w.txt
fi
