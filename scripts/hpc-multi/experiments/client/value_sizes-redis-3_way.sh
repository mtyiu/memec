...
if [ $w == "load" ]; then
	echo "-------------------- Load (workloada) --------------------"
	${BASE_PATH}/scripts-multi/ycsb/redis-3_way/load-size.sh $s 2>&1 | tee ${BASE_PATH}/results-multi/value_sizes/tmp/${HOSTNAME}/$s/load.txt
else
	echo "-------------------- Run ($w) --------------------"
	${BASE_PATH}/scripts-multi/ycsb/redis-3_way/run-size.sh $s $w 2>&1 | tee ${BASE_PATH}/results-multi/value_sizes/tmp/${HOSTNAME}/$s/$w.txt
fi
