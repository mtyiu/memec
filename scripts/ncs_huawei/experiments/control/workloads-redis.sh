...
for iter in {1..30}; do
	...
	for c in $coding; do
		...
		for t in $threads; do
			...
			screen -S manage -p 0 -X stuff "${HOME}/hwchan/util/redis/create-cluster-distributed-8 start$(printf '\r')"
			sleep 10
			screen -S manage -p 0 -X stuff "${HOME}/hwchan/util/redis/create-cluster-distributed-8 create$(printf '\r')"
			sleep 5
			...
			for w in $workloads; do
				...
				for n in {11..18}; do
					ssh testbed-node$n "screen -S ycsb -p 0 -X stuff \"${BASE_PATH}/scripts_huawei/experiments/master/workloads-redis.sh $c $t $w $(printf '\r')\"" &
				done
			...
			screen -S manage -p 0 -X stuff "${HOME}/hwchan/util/redis/create-cluster-distributed-8 stop$(printf '\r')"
			sleep 5
			screen -S manage -p 0 -X stuff "${HOME}/hwchan/util/redis/create-cluster-distributed-8 clean$(printf '\r')"
			sleep 10
