...
if [ $w == "load" ]; then
	echo "-------------------- Load (workloada) --------------------"
	${BASE_PATH}/scripts/ycsb/redis/load-value-$c.sh $v 2>&1 | tee ${BASE_PATH}/results/value-size/$c/$v/load.txt
else
	echo "-------------------- Run ($w) --------------------"
	${BASE_PATH}/scripts/ycsb/redis/run-value-$c.sh $v $w 2>&1 | tee ${BASE_PATH}/results/value-size/$c/$v/$w.txt
fi
