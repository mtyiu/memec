...
for s in $sizes; do
	for iter in {1..10}; do
		...
		scp testbed-node$n:${MEMEC_PATH}/memory.log ${BASE_PATH}/results/recovery/$s/$iter/
		scp testbed-node1:${MEMEC_PATH}/coordinator.log ${BASE_PATH}/results/recovery/$s/$iter/
	done
done
