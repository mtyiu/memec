...
for c in $clients; do
	for v in $value_sizes; do
		for iter in {1..10}; do
			mkdir -p ${BASE_PATH}/results/value-sizes/memec/$c/$v/$iter
			...
			for w in $workloads; do
				...
				for n in {2..5}; do
					ssh testbed-node$n "screen -S ycsb -p 0 -X stuff \"${BASE_PATH}/scripts/experiments/client/value-sizes-memec.sh $c $v $w $(printf '\r')\"" &
				done
			...
			for n in {2..5}; do
				mkdir -p ${BASE_PATH}/results/value-sizes/memec/$c/$v/$iter/node$n
				scp testbed-node$n:${BASE_PATH}/results/value-sizes/memec/$c/$v/*.txt ${BASE_PATH}/results/value-sizes/memec/$c/$v/$iter/node$n
