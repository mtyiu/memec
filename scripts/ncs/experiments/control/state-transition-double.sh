...
for delay in $delays; do
	for iter in {1..10}; do
		...
		#sleep 5
		#restore_overload
		#ssh testbed-node1 "screen -S coordinator -p 0 -X stuff \"remapping$(printf '\r')underload$(printf '\r')7$(printf '\r')18$(printf '\r')0$(printf '\r')\""
		#sleep 3
		#ssh testbed-node1 "screen -S coordinator -p 0 -X stuff \"remapping$(printf '\r')\""
		...
		for n in 3 4 8 9; do
			mkdir -p ${BASE_PATH}/results/transit-double/$delay/$iter/node$n
			scp testbed-node$n:${BASE_PATH}/results/transition/*.txt ${BASE_PATH}/results/transit-double/$delay/$iter/node$n
			ssh testbed-node$n "rm -rf ${BASE_PATH}/results/*"
		done
		scp testbed-node1:${MEMEC_PATH}/coordinator.log ${BASE_PATH}/results/transit-double/$delay/$iter/coordinator.log
	done
done
