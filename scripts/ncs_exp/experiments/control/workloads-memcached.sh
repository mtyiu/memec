	...
	for c in $coding; do
		...
		for t in $threads; do
			...
			for w in $workloads; do
				...
				for n in 3 4 8 9; do
					ssh testbed-node$n "screen -S ycsb -p 0 -X stuff \"${BASE_PATH}/scripts/experiments/master/workloads-memcached.sh $c $t $w $(printf '\r')\"" &
				done
			...
			for n in 3 4 8 9; do
				...
				scp testbed-node$n:${BASE_PATH}/results/workloads/$c/$t/*.csv ${BASE_PATH}/results/workloads/memcached/$iter/node$n
