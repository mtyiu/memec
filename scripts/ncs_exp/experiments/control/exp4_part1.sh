...
function set_slave {
	ssh testbed-node1 "screen -S coordinator -p 0 -X stuff \"manual$(printf '\r')overload$(printf '\r')6$(printf '\r')0$(printf '\r')\""
}

function unset_slave {
	ssh testbed-node1 "screen -S coordinator -p 0 -X stuff \"manual$(printf '\r')underload$(printf '\r')6$(printf '\r')0$(printf '\r')\""
}
...
DELAY_BASE="${i}us"
OUT_PATH="${BASE_PATH}/results/exp4_part1/${DELAY_BASE}/"
