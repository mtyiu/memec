...
function start_screens {
	...
	## COORDINATOR ##
	ssh testbed-node30 "if [ -z \"\`screen -ls | grep coordinator | grep -v grep\`\" ]; then echo \"[Node 30] Creating screen for coordinator\"; screen -dmS coordinator; else echo \"[Node 1] Coordinator screen exists\"; fi"
...
function stop_screens {
	...
	## COORDINATOR ##
	ssh testbed-node30 "tmpid=\`screen -ls | grep coordinator | awk '{print \$1}' | awk -F '.' '{print \$1}'\`; if [ -z \"\$tmpid\" ]; then echo \"[Node 30] No screen for coordinator\"; else echo \"[Node 1] Kill coordinator screen\"; kill \$tmpid; fi"
