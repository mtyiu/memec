...
for delay in $delays; do
	for iter in {1..10}; do
		...
		echo "-------------------- Load --------------------"
		for n in {31..34}; do
			ssh testbed-node$n "screen -S ycsb -p 0 -X stuff \"${MEMEC_PATH}/scripts/ncs-10g/experiments/client/temporary-timeseries.sh load $(printf '\r')\"" &
		done
		...
		for w in $workloads; do
			echo "-------------------- Run ($w) --------------------"
			for n in {31..34}; do
				ssh testbed-node$n "screen -S ycsb -p 0 -X stuff \"${MEMEC_PATH}/scripts/ncs-10g/experiments/client/temporary-timeseries.sh $w $(printf '\r')\"" &
			done
		...
		for n in {31..34}; do
			mkdir -p ${BASE_PATH}/results/temporary_control-timeseries/$delay/$iter/node$n
			scp testbed-node$n:${BASE_PATH}/results/temporary/*.txt ${BASE_PATH}/results/temporary_control-timeseries/$delay/$iter/node$n
