...
function set_manual {
	ssh testbed-node30 "screen -S coordinator -p 0 -X stuff \"manual$(printf '\r')\""
}
...
function unset_server {
	ssh testbed-node30 "screen -S coordinator -p 0 -X stuff \"underload$(printf '\r')7$(printf '\r')0$(printf '\r')\""
}
...
OUT_DIR=${BASE_PATH}/results/${OUT_NAME}/${DELAY_BASE}
