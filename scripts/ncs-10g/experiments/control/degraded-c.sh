...
for delay in $delays; do
	for iter in {1..10}; do
		...
		for n in {31..34}; do
			mkdir -p ${BASE_PATH}/results/degraded-c/$delay/$iter/node$n
			scp testbed-node$n:${BASE_PATH}/results/degraded/*.txt ${BASE_PATH}/results/degraded-c/$delay/$iter/node$n
