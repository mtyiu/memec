...
for iter in {1..10}; do
	...
	for c in $coding; do
		...
		sed -i "s/^scheme=.*$/scheme=$c/g" ${MEMEC_PATH}/bin/config/aws/global.ini
		...
		for t in $threads; do
			...
			for w in $workloads; do
				...
				for n in {1..4}; do
					ssh client "screen -S ycsb${n} -p 0 -X stuff \"${BASE_PATH}/scripts/experiments/client/workloads-memec.sh $c $t $w $(printf '\r')\"" &
				done
...
sed -i "s/^scheme=.*$/scheme=raid0/g" ${MEMEC_PATH}/bin/config/aws/global.ini
