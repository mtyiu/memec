...
for s in $sizes; do
	for iter in {1..10}; do
		...
		ssh node$n "screen -S slave -p 0 -X stuff \"$(printf '\r\r')sync$(printf '\r\r')\""
		sleep 5
		ssh node$n "screen -S slave -p 0 -X stuff \"$(printf '\r\r')memory$(printf '\r\r')\""
		sleep 1
		ssh node$n "killall -9 slave; screen -S slave -p 0 -X stuff \"$(printf '\r\r')\""
