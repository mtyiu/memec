...
for delay in $delays; do
	for iter in {1..10}; do
		...
		ssh node1 "screen -S coordinator -p 0 -X stuff \"overload$(printf '\r')7$(printf '\r')0$(printf '\r')\""
		set_overload $delay
		...
		for n in {1..4}; do
			mkdir -p ${BASE_PATH}/results/degraded/$delay/$iter/client-ycsb$n
			scp client:${BASE_PATH}/results/client-ycsb${n}/degraded/*.txt ${BASE_PATH}/results/degraded/$delay/$iter/client-ycsb${n}
