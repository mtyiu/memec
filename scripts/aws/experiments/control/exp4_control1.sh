...
# for iter in {1..30}; do
	...
	echo "-------------------- Load --------------------"
	for n in {1..4}; do
		ssh node$n "screen -S ycsb -p 0 -X stuff \"${BASE_PATH}/scripts/experiments/master/degraded.sh load $(printf '\r')\"" &
	done
	...
	for w in $workloads; do
		for n in {1..4}; do
			ssh node$n "screen -S ycsb -p 0 -X stuff \"${BASE_PATH}/scripts/experiments/master/degraded.sh $w $(printf '\r')\"" &
		done
	...
	for n in {1..4}; do
		mkdir -p ${BASE_PATH}/results/exp4_control1/$iter/node$n
		scp node$n:${BASE_PATH}/results/degraded/*.txt ${BASE_PATH}/results/exp4_control1/$iter/node$n
