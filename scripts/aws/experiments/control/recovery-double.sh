...
for s in $sizes; do
	for iter in {1..10}; do
		...
		scp node$n1:${PLIO_PATH}/memory.log ${BASE_PATH}/results/recovery-double/$s/$iter/memory-${n1}.log
		scp node$n2:${PLIO_PATH}/memory.log ${BASE_PATH}/results/recovery-double/$s/$iter/memory-${n2}.log
		scp node1:${PLIO_PATH}/coordinator.log ${BASE_PATH}/results/recovery-double/$s/$iter/
	done
done
