...
if [ $w == "load" ]; then
	...
	${BASE_PATH}/scripts/ycsb/memcached/load.sh $t 2>&1 | tee ${BASE_PATH}/results/workloads/$c/$t/load.txt
else
	echo "-------------------- Run ($w) --------------------"
	# ${BASE_PATH}/scripts/ycsb/plio/run.sh $t $w 2>&1 | tee ${BASE_PATH}/results/workloads/$c/$t/$w.txt
	${BASE_PATH}/scripts/ycsb/memcached/run.sh $t $w 2>&1 | tee ${BASE_PATH}/results/workloads/$c/$t/$w.txt
fi
