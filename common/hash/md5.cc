...
char *MD5::getHash( char *hashBuffer )
{
...
uint32_t *MD5::getHash( uint32_t *ret )
{
	// save old hash if buffer is partially filled
	uint32_t oldHash[4];
	oldHash[0] = m_hash[0];
...
char *MD5::operator()(const void* data, size_t numBytes,
											char *hashBuffer )
{
	reset();
	add(data, numBytes);
	return getHash( hashBuffer );
}

char *MD5::toString( const uint32_t *input, char *output ) {
	static const char dec2hex[16+1] = "0123456789abcdef";

	output[ 0] = dec2hex[(input[0] >>  4) & 15];
	output[ 1] = dec2hex[ input[0]        & 15];
	output[ 2] = dec2hex[(input[0] >> 12) & 15];
	output[ 3] = dec2hex[(input[0] >>  8) & 15];
	output[ 4] = dec2hex[(input[0] >> 20) & 15];
	output[ 5] = dec2hex[(input[0] >> 16) & 15];
	output[ 6] = dec2hex[ input[0] >> 28      ];
	output[ 7] = dec2hex[(input[0] >> 24) & 15];

	output[ 8] = dec2hex[(input[1] >>  4) & 15];
	output[ 9] = dec2hex[ input[1]        & 15];
	output[10] = dec2hex[(input[1] >> 12) & 15];
	output[11] = dec2hex[(input[1] >>  8) & 15];
	output[12] = dec2hex[(input[1] >> 20) & 15];
	output[13] = dec2hex[(input[1] >> 16) & 15];
	output[14] = dec2hex[ input[1] >> 28      ];
	output[15] = dec2hex[(input[1] >> 24) & 15];

	output[16] = dec2hex[(input[2] >>  4) & 15];
	output[17] = dec2hex[ input[2]        & 15];
	output[18] = dec2hex[(input[2] >> 12) & 15];
	output[19] = dec2hex[(input[2] >>  8) & 15];
	output[20] = dec2hex[(input[2] >> 20) & 15];
	output[21] = dec2hex[(input[2] >> 16) & 15];
	output[22] = dec2hex[ input[2] >> 28      ];
	output[23] = dec2hex[(input[2] >> 24) & 15];

	output[24] = dec2hex[(input[3] >>  4) & 15];
	output[25] = dec2hex[ input[3]        & 15];
	output[26] = dec2hex[(input[3] >> 12) & 15];
	output[27] = dec2hex[(input[3] >>  8) & 15];
	output[28] = dec2hex[(input[3] >> 20) & 15];
	output[29] = dec2hex[(input[3] >> 16) & 15];
	output[30] = dec2hex[ input[3] >> 28      ];
	output[31] = dec2hex[(input[3] >> 24) & 15];
...
uint32_t *MD5::toArray( const char *inputConst, uint32_t *output ) {
	char input[ 32 ];
	for ( int i = 0; i < 32; i++ ) {
		input[ i ] = inputConst[ i ];
