...
bool Protocol::parseStripeListScalingHeader( struct StripeListScalingHeader &header, char *buf, size_t size, size_t offset ) {
	if ( ! buf || ! size ) {
...
bool Protocol::parseStripeListPartitionHeader( struct StripeListPartitionHeader &header, uint32_t n, uint32_t k, char *buf, size_t size, size_t offset ) {
	if ( ! buf || ! size ) {
		buf = this->buffer.recv;
		size = this->buffer.size;
	}
