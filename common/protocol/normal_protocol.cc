...
bool Protocol::parseChunkKeyValueUpdateHeader( struct ChunkKeyValueUpdateHeader &header, bool withValueUpdate, char *buf, size_t size, size_t offset ) {
	...
	header.key = ptr;
	header.valueUpdate = withValueUpdate && header.valueUpdateSize ? header.key + header.keySize : 0;
	return ( size - offset >= PROTO_CHUNK_KEY_VALUE_UPDATE_SIZE + header.keySize + ( withValueUpdate ? header.valueUpdateSize : 0 ) );
}
...
bool Protocol::parseKeyValueHeader( struct KeyValueHeader &header, char *buf, size_t size, size_t offset ) {
	...
	header.key       = ptr;
	header.value     = header.valueSize ? header.key + header.keySize : 0;
